...
const Analytics = () => {
  ...
  const [customReports, setCustomReports] = useState<CustomReport[]>([]);
  const [showCustomReportConfig, setShowCustomReportConfig] = useState(false);
  const [selectedCustomReport, setSelectedCustomReport] = useState<CustomReport | null>(null); // ⭐ NEW

  useEffect(() => {
    const savedReports = localStorage.getItem('customReports');
    if (savedReports) {
      setCustomReports(JSON.parse(savedReports));
    }
  }, []);
  ...
  const handleSaveCustomReport = (report: CustomReport) => {
    setCustomReports(prev => {
      ...
      if (exists) {
        return prev.map(r => r.id === report.id ? report : r); // ⭐ MODIFIED: update existing
      } else {
        return [...prev, report]; // ⭐ MODIFIED: add new
      }
    });
    setShowCustomReportConfig(false);
    setSelectedCustomReport(report); // ⭐ NEW
    setReportType('custom'); // ⭐ NEW
  };
  ...
  const confirmDeleteReport = (report: CustomReport) => {
    if (window.confirm(`Are you sure you want to delete "${report.name}"?`)) {
      ...
      if (selectedCustomReport?.id === report.id) {
        setSelectedCustomReport(null); // ⭐ NEW
        setReportType('weekly');
      }
    }
  };
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            {customReports.map(report => (
              ...
              >
                <span>{report.name}</span>
                <div className="flex items-center gap-2 ml-auto"> {/* ⭐ NEW */}
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8"
                    onClick={(e) => {
                      e.stopPropagation();
                      setSelectedCustomReport(report); // ⭐ NEW
                      setShowCustomReportConfig(true); // ⭐ NEW
                    }}
                  >
        ...
        <div className={cn('ml-auto flex items-center space-x-2')}>
      ...
      {reportType === 'custom' && selectedCustomReport && (
        <div className="border p-4 rounded-xl shadow-sm">
          <h2 className="text-lg font-semibold mb-2">{selectedCustomReport.name}</h2>
          <pre className="text-sm text-gray-700">
            {JSON.stringify(selectedCustomReport.filters, null, 2)}
          </pre>
        </div>
      )}
      ...
      {showCustomReportConfig && (
        <CustomReportConfig
          existingReport={selectedCustomReport} // ⭐ NEW
          onSave={handleSaveCustomReport}
          onCancel={() => setShowCustomReportConfig(false)}
        />
      )}
