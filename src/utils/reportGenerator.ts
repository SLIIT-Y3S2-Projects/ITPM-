...
export const generateDashboardReport = (
  ...
  const completionRate = tasks.length ? Math.round((analytics.completedTasks / tasks.length) * 100) : 0;
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Upcoming Reminders', reminders.filter(r => !r.isCompleted).length.toString()],
    ],
    theme: 'grid',
...
export const generateTasksReport = (
  ...
  filteredTasks: Task[],
  userName: string,
  filters: { status: string | null; priority: string | null; category: string | null; searchQuery: string }
) => {
  const doc = new jsPDF();
  ...
  autoTable(doc, {
    ...
    body: [
      ['Completed', completedCount.toString(), `${tasks.length ? Math.round((completedCount / tasks.length) * 100) : 0}%`],
      ['Pending', pendingCount.toString(), `${tasks.length ? Math.round((pendingCount / tasks.length) * 100) : 0}%`],
      ['In Progress', inProgressCount.toString(), `${tasks.length ? Math.round((inProgressCount / tasks.length) * 100) : 0}%`],
      ['Missed', missedCount.toString(), `${tasks.length ? Math.round((missedCount / tasks.length) * 100) : 0}%`]
    ],
    theme: 'grid',
    headStyles: { fillColor: [0, 77, 153] }
  });
...
export const generateRemindersReport = (
  ...
  const upcomingCount = reminders.filter(r => !r.isCompleted && new Date(r.date) > new Date()).length;
  const overdueCount = reminders.filter(r => !r.isCompleted && new Date(r.date) < new Date()).length;
  
  autoTable(doc, {
    startY: 81,
    head: [['Status', 'Count', 'Percentage']],
    body: [
      ['Completed', completedCount.toString(), `${reminders.length ? Math.round((completedCount / reminders.length) * 100) : 0}%`],
      ['Upcoming', upcomingCount.toString(), `${reminders.length ? Math.round((upcomingCount / reminders.length) * 100) : 0}%`],
      ['Overdue', overdueCount.toString(), `${reminders.length ? Math.round((overdueCount / reminders.length) * 100) : 0}%`]
    ],
    theme: 'grid',
    headStyles: { fillColor: [0, 77, 153] }
  });
  ...
  if (filteredReminders.length > 0) {
    autoTable(doc, {
      ...
      body: filteredReminders.map(reminder => [
        ...
        reminder.isCompleted ? 'Completed' : new Date(reminder.date) > new Date() ? 'Upcoming' : 'Overdue',
        reminder.type
      ]),
      theme: 'grid',
      headStyles: { fillColor: [0, 77, 153] }
...
export const generateAnalyticsReport = (
  ...
  finalY += 6;
  const topCategory = categoryData.sort((a, b) => Number(b[1]) - Number(a[1]))[0]?.[0] || 'Work';
  doc.text(`You have a higher concentration of ${topCategory} tasks.`, 20, finalY);
  finalY += 6;
  doc.text('Consider balancing your workload across different categories for better productivity.', 20, finalY);
  finalY += 10;
  ...
  if (completionRate > 70) {
    ...
    doc.text('is good, but could be improved. Try breaking down larger tasks into smaller ones.', 20, finalY);
  } else {
    doc.text('needs improvement. Consider using more reminders or adjusting your workload.', 20, finalY);
  }
  finalY += 10;
  ...
  finalY += 6;
  doc.text('Based on your completion patterns, the best time to schedule important tasks is', 20, finalY);
  finalY += 6;
  doc.text('during midweek (Tuesday-Thursday), when your productivity appears to be at its highest.', 20, finalY);
  ...
  if (isCustomReport) {
    ...
    finalY += 6;
    doc.text(`This report focuses on the "${reportType.substring(7)}" category selection.`, 20, finalY);
  }
